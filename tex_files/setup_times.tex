...
Observe next that the machine not only  serves jobs, part of the time it is  occupied with setups. This leads to the idea of  \emph{incorporating} the effects of the setup times in the service times. For this we distinguish between a job's \recall{net service time} $S_0$  and its \recall{effective processing time} $S$ which also include setup times. 
